...
app.post("/webhook/shopify-events", (req, res) => {
    try {
...
const SHOPIFY_APP_CONFIG = {
    clientId: "181a611e074aaa1904c7ed0b88ce4952", // Your client ID from app.toml
...
if (!SHOPIFY_APP_CONFIG.clientSecret) {
    console.error('❌ SHOPIFY_CLIENT_SECRET environment variable is required for real token exchange');
    console.log('💡 Set it in Vercel dashboard or locally: export SHOPIFY_CLIENT_SECRET="your-secret"');
}
// Install shopify using app store - simplified mock version
app.get('/shopify/install/direct', (req, res) => {
    const shop = req.query.shop || 'nabiqtesting.myshopify.com'; // Mock shop
...
app.get('/shopify/auth/callback', async (req, res) => {
    try {
        ...
        if (tokenData.associated_user) {
            console.log(`👤 User: ${tokenData.associated_user.first_name} ${tokenData.associated_user.last_name} (${tokenData.associated_user.email})`);
        }
        console.log('===============================================');
        // In a real app, you'd:
